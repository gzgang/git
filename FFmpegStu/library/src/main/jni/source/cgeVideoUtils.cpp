...
namespace CGE
{
    ...
    bool cgeGenerateVideoWithFilter(const char* outputFilename, const char* inputFilename, const char* filterConfig, float filterIntensity, GLuint texID, CGETextureBlendMode blendMode, float blendIntensity, bool mute, CGETexLoadArg* loadArg)
    {
        ...
        if(!mp4Encoder.init(outputFilename, ENCODE_FPS, videoWidth, videoHeight, !mute, (int)(1.5 * 1024 * 1024), audioSampleRate))
        {
        ...
        JNIEnv* jniEnv = loadArg->env;
        jclass javaNativeClass = loadArg->cls;
        jmethodID javaFilterProgressCallbackMethod = (*jniEnv).GetStaticMethodID(javaNativeClass,"generateVideoCallback","(I)V");

        double totalTime = decodeHandler->getTotalTime() / 1000;
        ...
        while(1)
        {
            ...
            if(nextFrameType == FrameType_VideoFrame)
            {
                ...
                double timestamp = decodeHandler->getCurrentTimestamp();
                int percent = (int)(timestamp * 100 / totalTime);
                (*jniEnv).CallStaticVoidMethod(javaNativeClass, javaFilterProgressCallbackMethod, percent);

                int newPTS = round(timestamp / 1000.0 * ENCODE_FPS);
                
                CGE_LOG_INFO("last pts: %d, new pts; %d, timestamp: %d%%\n", videoPTS, newPTS,percent);
        ...
        (*jniEnv).CallStaticVoidMethod(javaNativeClass, javaFilterProgressCallbackMethod, 100);
        return true;
    }
